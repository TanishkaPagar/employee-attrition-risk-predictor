...
st.markdown(
    '<div class="sub-title">Powered by XGBoost + OneHotEncoding + SHAP Explainability</div>',
    unsafe_allow_html=True
)
...
with st.expander("📊 Model Performance Metrics", expanded=False):
    ...
    st.success(
        f"✅ Best Method Selected: **{model_data['best_sampler']}**"
    )
...
with tab1:
    ...
    if st.button(
        ...
        with st.spinner("Analyzing employee profile..."):
            ...
            prob, risk_label, factors = predict_single(
                input_data,
                model_data
            )
        ...
        with col_res2:
            ...
            if "High" in risk_label:
                ...
                st.error(
                    "⚠️ High Risk! Immediate retention action recommended."
                )

            elif "Medium" in risk_label:

                st.warning(
                    "👀 Medium Risk. Schedule a check-in with this employee."
                )
...
with tab2:
    ...
    st.subheader(
        "Bulk Employee Attrition Analysis"
    )

    st.info(
        "Upload a CSV with IBM HR dataset columns to score all employees at once."
    )
